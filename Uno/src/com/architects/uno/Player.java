...
public class Player extends Actor implements IPlayer {
    ...
    public void renderCards() {
        GreenfootImage image = new GreenfootImage(CARD_GAP * this.cards.size()+144, 196);
        //GreenfootImage image = new GreenfootImage(this.CARD_GAP * this.cards.size() + CARD_WIDTH, 144);
        int x = CARD_WIDTH /2;
        for (Card card: this.cards) {
            if (GameScreen.showEnemyCards || isHuman()) {
                image.drawImage(card.getImage(), x, 1);
            } else {
